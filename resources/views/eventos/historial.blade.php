                ...
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        <div class="bg-white p-6 rounded-lg shadow-sm">
                            ...
                            <p class="mt-2 text-3xl font-semibold text-gray-900">{{ $metricas['totalInvitados'] }} <span
                                    class="text-lg font-normal">totales</span></p>
                            <p class="mt-1 text-3xl font-semibold text-green-600">{{ $metricas['invitadosIngresaron'] }}
                                <span class="text-lg font-normal">ingresaron</span></p>
                            ...
                            <p class="mt-1 text-3xl font-semibold text-blue-600">
                                {{ $metricas['beneficios']['Entrada Free'] ?? 0 }} <span
                                    class="text-lg font-normal">ingresaron free</span></p>
                            {{-- Quiero agregar una metrica que indique los que no ingresaron --}}
                            <p class="mt-1 text-3xl font-semibold text-red-600">
                                {{ $metricas['totalInvitados'] - $metricas['invitadosIngresaron'] }} <span
                                    class="text-lg font-normal">no ingresaron</span></p>
                        </div>
                        ...
                        <div class="bg-white p-6 rounded-lg shadow-sm">
                            ...
                            <p class="mt-2 text-3xl font-semibold text-emerald-600">
                                ${{ number_format($metricas['ingresosEstimados'], 2, ',', '.') }}</p>
                            <p class="text-sm text-gray-500 mt-1">Precio entrada:
                                ${{ number_format($eventoSeleccionado->precio_entrada, 2, ',', '.') }}</p>
                        </div>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 bg-white border-b border-gray-200">
                        <h3 class="text-lg font-bold mb-4">
                            Mostrando {{ $invitados->count() }} invitados para el evento del
                            {{ \Carbon\Carbon::parse($eventoSeleccionado->fecha_evento)->format('d/m/Y') }}
                        </h3>
                                        ...
                                        <tr class="{{ $invitado->ingreso ? 'bg-green-50' : '' }}">
                                            ...
                                            <td class="px-6 py-4 text-base text-gray-500">
                                                @forelse($invitado->beneficios as $beneficio)
                                                    <span
                                                        class="inline-block bg-blue-100 text-blue-800 rounded-full px-2 py-1 text-xs font-semibold mr-1 mb-1">
                                                        {{ $beneficio->nombre_beneficio }}
                                                        ({{ $beneficio->pivot->cantidad }})
                                                    </span>
                                                @empty
            ...
            @if ($eventoIdSeleccionado && !$eventoSeleccionado)
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 bg-white border-b border-gray-200 text-center text-gray-500">
                        El evento seleccionado no es válido o no tiene invitados. Por favor, selecciona otro de la
                        lista.
                    </div>
                </div>
            @endif
