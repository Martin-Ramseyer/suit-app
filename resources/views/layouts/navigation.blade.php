<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            {{-- ================= ENLACES RESPONSIVE SOLO PARA ADMIN ================= --}}
            @if(Auth::user()->rol === 'ADMIN')
                ...
                {{-- NUEVO ENLACE RESPONSIVE: Historial de Eventos --}}
                <x-responsive-nav-link :href="route('eventos.historial')" :active="request()->routeIs('eventos.historial')">
                    {{ __('Historial Eventos') }}
                </x-responsive-nav-link>
            @endif

            {{-- ================= ENLACE RESPONSIVE PARA RRPP, ADMIN, CAJERO ================= --}}
            @if(in_array(Auth::user()->rol, ['RRPP', 'ADMIN', 'CAJERO']))
